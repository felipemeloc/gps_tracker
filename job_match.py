...
def get_match_report(completed_jobs:pd.DataFrame, travel_data:pd.DataFrame)->pd.DataFrame:
    ...
    if match:
        ...
        return pd.DataFrame(columns=['Position A', 'Left', 'Duration', 'Route length (Mi)',
                                    'Position B',	'End',	'Time at location',	'Departure time',
                                    'Average speed (mph)',	'Top speed (mph)',	'Locksmith', 'ReportID', 
                                    'Distance (Mi)', 'Service'])
